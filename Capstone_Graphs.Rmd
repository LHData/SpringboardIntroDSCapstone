...
```{r, echo=FALSE}
ggplot(stop_type_by_ethnicity_citywide_all_years, aes(x= ethnicity, y = ethnicity_percent)) + geom_bar(aes(fill=stop_type), stat= 'identity', position='dodge')
#ggplot(ethnicity_by_stop_type_citywide_all_years, aes(x= ethnicity, y = ethnicity_percent)) + geom_bar(aes(fill=stop_type), stat= 'identity', position='dodge')
...
stops_summary_all_years_with_pop %>% 
  ...
  geom_point() +
  geom_smooth(method = 'lm', formula = y~x) + 
  ggtitle("Black counts by divsion ")
...
stops_summary_all_years_with_pop %>% 
  ...
  ggplot(aes(x=population, y=stops)) + 
    geom_point() +
    geom_smooth(method = 'lm', formula = y~x) + 
  ggtitle("Black percentages by divsion ")

stops_summary_by_year_with_pop %>%
  filter(division != "CITY TOTAL", year == 2010 | location == "population", ethnicity == "BLACK") %>%
  select(1,3,5,6) %>%
  spread(key = location, value = ethnicity_percent) %>%
  ggplot(aes(x=population, y=stops, col = division)) + 
    geom_point() + ylim(0,.7) +
  ggtitle("Black percentages by divsion 2010")
...
stops_summary_by_year_with_pop %>%
  ...
  ggplot(aes(x=population, y=stops, col = division)) + 
    geom_point() + ylim(0,.7) +
  ggtitle("Black percentages by divsion 2018")
