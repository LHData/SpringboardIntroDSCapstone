...
stops_summary_by_year_with_pop %>% 
  filter(division == "CITY TOTAL", ethnicity == "BLACK", location == "stops") %>%
...
Try another graph. Plot population percentage verses stops percentage by division for blacks.
...
black_div_sum <- left_join(black_div_pop, black_div_stops, by = "division") %>%
  ...
  mutate(stop_to_pop_ratio = stop_count / pop_count)
